...
int main(int argc, char** argv) { 
  ...
  factory->AddVariable( "boostedDphill_130" , 'F');
...
//   factory->AddVariable( "jettche1" , 'F');
//   factory->AddVariable( "jettche2" , 'F');

  
//   factory->AddVariable( "boostedDphill_160" , 'F');
...
//   factory->SetSignalWeightExpression    ( nameWeight.c_str() );
  ...
  if (Use["CutsGA"])   factory->BookMethod( TMVA::Types::kCuts, "CutsGA","H:!V:FitMethod=GA:EffSel:Steps=30:Cycles=3:PopSize=400:SC_steps=10:SC_rate=5:SC_factor=0.95" );
...
//    TString theCatVars = "jetpt1:jetpt2:mjj:detajj:dphilljetjet:pt1:pt2:mll:dphill:mth:dphillmet:mpmet";      
   TString theCatVars = "jetpt1:jetpt2:mjj:detajj:dphilljetjet:pt1:pt2:mll:boostedDphill_130:mth:dphillmet:mpmet:drll";      
...
//    TString theCatVars = "jetpt1:jetpt2:mjj:detajj:dphilljetjet:pt1:pt2:mll:dphill:mth:dphillmet:mpmet";      
   TString theCatVars = "jetpt1:jetpt2:mjj:detajj:dphilljetjet:pt1:pt2:mll:boostedDphill_130:mth:dphillmet:mpmet:drll";      
   mcat->AddMethod( "abs(channel-0)<0.5",theCatVars, TMVA::Types::kBDT, "Category_BDT_mm","!H:!V:VarTransform=D,G,N:NTrees=700:nEventsMin=150:MaxDepth=5:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=50:PruneMethod=NoPruning" );
   mcat->AddMethod( "abs(channel-3)<0.5",theCatVars, TMVA::Types::kBDT, "Category_BDT_me","!H:!V:VarTransform=D,G,N:NTrees=700:nEventsMin=150:MaxDepth=5:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=50:PruneMethod=NoPruning" );
   mcat->AddMethod( "abs(channel-2)<0.5",theCatVars, TMVA::Types::kBDT, "Category_BDT_em","!H:!V:VarTransform=D,G,N:NTrees=700:nEventsMin=150:MaxDepth=5:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=50:PruneMethod=NoPruning" );
   mcat->AddMethod( "abs(channel-1)<0.5",theCatVars, TMVA::Types::kBDT, "Category_BDT_ee","!H:!V:VarTransform=D,G,N:NTrees=700:nEventsMin=150:MaxDepth=5:BoostType=AdaBoost:AdaBoostBeta=0.5:SeparationType=GiniIndex:nCuts=50:PruneMethod=NoPruning" );
  }
...
//   factory->OptimizeAllMethods("ROCIntegral","Scan");
  ...
  toDo = "rm -r Weights-MVA-VH/weights_" + HiggsMass + "_testVariables";
  std::cerr << "toDo = " << toDo << std::endl;
  system (toDo.c_str()); 
  
  toDo = "mv weights Weights-MVA-VH/weights_" + HiggsMass + "_testVariables";
  std::cerr << "toDo = " << toDo << std::endl;
  system (toDo.c_str()); 
