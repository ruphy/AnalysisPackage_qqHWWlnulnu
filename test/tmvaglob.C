...
namespace TMVAGlob {
   ...
   std::vector<TString> GetInputVariableNames(TDirectory *dir )
   {
	 ...
	 std::string tempString = name.Data();
//          Int_t pos = name.First("__");
	 Int_t pos = tempString.find("__");
	 Int_t posTemp = tempString.find("___");
	 if (posTemp != -1) pos += 1;
// 	 std::cerr << " name = " << name.Data() << std::endl;
//          std::cerr << " pos = " << pos << std::endl;
	 name.Remove(pos);
//      	 std::cerr << " name after remove = " << name.Data() << std::endl;
