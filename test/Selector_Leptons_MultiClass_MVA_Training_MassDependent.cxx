...
void Selector_Leptons_MultiClass_MVA_Training_MassDependent( TString myMethodList = "" , std::string HiggsMass = "qqH160") {
 ...
 TString outfileName = Form( "output/Selector_Leptons_trained_multiclass_%s.root", HiggsMass.c_str() );
 ...
 char nameFileIn[1000] = {"test/Winter10/samples_skimmed_training_AllHiggsMasses_Multiclass.txt"};
 ...
 for (int iSample=0; iSample<totalSamples; iSample++){
  ...
  if (numEntriesBefore != 0) weights[iSample] = XSection * preselection_efficiency / numEntriesBefore;
  else {
   std::cout << " >>> how is it possible that numEntriesBefore == 0 ?" << std::endl;   
  }
...
//    factory->OptimizeAllMethods("ROCIntegral","Scan");
   ...
   std::string toDo = "mv weights Weights/weights_multiclass_leptons_" + HiggsMass;
