...
// define map with events
 ...
 int kind = 1;
 try {
  kind = gConfigParser -> readIntOption("Options::kind");
 }
 catch (char const* exceptionString){
  std::cerr << " exception = " << exceptionString << std::endl;
  kind = 1;
 }
 std::cout << ">>>>> Options::kind  " << kind  << std::endl;  
...
if (debug) std::cerr << " q1 = " << q1 << " : q2 = " << q2 << std::endl;
  ...
  //  jecUnc->setJetEta(ivars.q1_Eta);
  //  jecUnc->setJetPt(vars.q1_pT);
  //  double unc1 = jecUnc->getUncertainty(true);
  
  
  double unc1 = (kind) *   JESUnc(vars.q1_Eta,vars.q1_pT);
  ...
  //  jecUnc->setJetEta(ivars.q2_Eta);
  //  jecUnc->setJetPt(vars.q2_pT);
  //  double unc2 = jecUnc->getUncertainty(true);
  
  double unc2 = (kind) * JESUnc(vars.q2_Eta,vars.q2_pT);
  ...
  vars.M_qq = (reader.Get4V("jets")->at(q1) *(1+unc1) + reader.Get4V("jets")->at(q2) * (1+unc2)).mass();
  
  vars.CJV_20 = getCJVJES(*(reader.Get4V("jets")),q1,q2,20.,&blacklistJet_forCJV, kind);
  vars.CJV_30 = getCJVJES(*(reader.Get4V("jets")),q1,q2,30.,&blacklistJet_forCJV, kind);
