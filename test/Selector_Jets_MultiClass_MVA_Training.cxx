...
void Selector_Jets_MultiClass_MVA_Training( TString myMethodList = "" ) {
 ...
 TString outfileName( "output/Selector_MultiClass_Jets_trained.root" );
 TFile* outputFile = TFile::Open( outfileName, "RECREATE" );
 
 //==== Create the factory ====
 TMVA::Factory *factory = new TMVA::Factory( "TMVAMulticlass", outputFile, Form("AnalysisType=multiclass:!V:!Silent:%sColor:!V:Transformations=I;D;P;G", gROOT->IsBatch()?"!":"") );
 ...
 factory->AddVariable( "q1_pT" , 'F');
 factory->AddVariable( "q2_pT" , 'F');
...
//  factory->AddSpectator( "q2_bTag_trackCountingHighPurBJetTags" , 'F');
 ...
 char nameFileIn[1000] = {"test/Winter10/samples_skimmed_training_multiclass.txt"};
 ...
 for (int iSample=0; iSample<totalSamples; iSample++){
//   if (iSample == 0)   factory->AddTree( signal_background[iSample], "Signal", weights[iSample] );
//   else
   factory->AddTree( signal_background[iSample], TString(nameHumanReadable[iSample]), weights[iSample] );
 }
 ...
 TString mycuts = Form("1");
 
 TCut mycut = mycuts;

 factory->PrepareTrainingAndTestTree( mycut,"SplitMode=Random:NormMode=NumEvents:!V");
 ///============> only ONE cut since all classes are the same for the training!!!
...
// 			 "H:!V:Formula=(0)+(1)*x0+(2)*x1+(3)*x2+(4)*x3:ParRanges=(-1,1);(-10,10);(-10,10);(-10,10);(-10,10):FitMethod=GA:PopSize=300:Cycles=3:Steps=20:Trim=True:SaveBestGen=1" );
      ...
      factory->BookMethod( TMVA::Types::kFDA, "FDA_GA", "H:!V:Formula=(0)+(1)*x0+(2)*x1+(3)*x2+(4)*x3:ParRanges=(-1,1);(-10,10);(-10,10);(-10,10);(-10,10):FitMethod=GA:PopSize=300:Cycles=3:Steps=20:Trim=True:SaveBestGen=1" );
		 ...
		 if (Use["MLPCat"]) {
		  ...
		  // ---- define categories ----
			  ...
			  TString theCat1Vars = "q1_pT:q2_pT:abs(q1_Eta):abs(q2_Eta):q1_Eta*q2_Eta:DEta_qq:DPhi_qq:M_qq:CJV_30";      
      ...
      factory->BookMethod( TMVA::Types::kBDT, "BDTG", 
             ...
             if (Use["BDTGNew"]) // gradient boosted decision trees
      factory->BookMethod( TMVA::Types::kBDT, "BDTGNew",                       
