...
//   gROOT->ProcessLine(toLoad.Data());
 ...
 //  [iName]
 TTree *treeEffVect[131];
 TTree *treeJetLepVect[131];
 

 
  //  [iCut][iVar] 
 TString* infoString[40][10];
 TLatex *infoLatex[40][10]; 
 TCanvas* ccCanvas[40][10];
 TCanvas* ccCanvasNormalize[40][10];
 TCanvas* ccCanvasPull[40][10];
 TCanvas* ccCanvasVarAndPull[40][10];
 TCanvas* ccCanvasPullTrace[40][10];
 TH1F* histoSumMC[40][10];
 //  [iName][iCut][iVar]
 TH1F* histo[131][40][10];
 TH1F* histo_temp[131][40][10];
 TH1F* histo_temp_over_under[131][40][10];
 
 //  [iName][iCut]
 double numEvents[131][40];
...
//  TPie* hTrendPie[40];
 ...
 THStack* hsSignal[40][131][30];
 THStack* hs[40][131];
 TH1F* hPull[40][131];
 TH1F* hPullTrace[40][131];

 TGraphErrors* grPull[40][131];
 TGraphErrors* grPullMC[40][131];
   ...
   for (unsigned int iName=0; iName<reduced_name_samples.size(); iName++){
    ...
    if ((!isSig || Discovery==0) && reduced_name_samples.at(iName) != "DATA") {
     ...
     if ((!isSig || Discovery==0)) {
      ...
      histo[iName][iCut][iVar]->SetMarkerStyle(10);
...
//  cTrendPieAll -> Divide (vCut.size());
 ...
 TCanvas* cCompareCutPull[40];
 TCanvas* cCompareVarPull[131];
 
 TCanvas* cCompareCut[40];
 TCanvas* cCompareVar[131];
